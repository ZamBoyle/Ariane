...
settings-text-size-select =
    .aria-label = Size of the text throughout the app
settings-update-now = Check now
update-now-checking = Checking…
update-now-current = Ariane { $version } is up to date.
update-now-failed = GitHub did not answer: try again later.
...
stats-records = The sidebar counts { $records } records: { $empty } of them hold nothing to show.
