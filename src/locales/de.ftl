...
convo-span =
    .title =
        ...
        Letzte Nachricht: { $end }
convo-duration =
    .title = Zeit von der ersten bis zur letzten Nachricht
convo-message-count = { $n ->
        [one] { $n } Nachricht
       *[other] { $n } Nachrichten
    }
convo-part = Teil { $n } von { $total }
    .title = Diese Unterhaltung wird in einem weiteren Transkript fortgesetzt: komprimiert, fortgeführt oder dupliziert
convo-part-previous =
...
settings-text-size-select =
    .aria-label = Größe des Textes in der ganzen App
settings-update-now = Jetzt prüfen
update-now-checking = Wird geprüft…
update-now-current = Ariane { $version } ist aktuell.
update-now-failed = GitHub hat nicht geantwortet: Versuchen Sie es später erneut.
