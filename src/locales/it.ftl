...
convo-span =
    .title =
        ...
        Ultimo messaggio: { $end }
convo-duration =
    .title = Tempo dal primo messaggio all’ultimo
convo-message-count = { $n ->
        [one] { $n } messaggio
       *[other] { $n } messaggi
    }
convo-part = Parte { $n } di { $total }
    .title = Questa conversazione prosegue in un'altra trascrizione: compattata, ripresa o duplicata
convo-part-previous =
...
convo-copied = { $count ->
    [one] Questa conversazione è iniziata copiando un messaggio da «{ $title }»: si legge lì e non viene ripetuto qui.
...
usage-floor = La trascrizione di un subagente non conserva sempre il suo ultimo conteggio: queste cifre sono un minimo.
...
settings-text-size = Dimensione del testo
settings-text-size-select =
    .aria-label = Dimensione del testo in tutta l'app
settings-update-now = Controlla ora
update-now-checking = Controllo in corso…
update-now-current = Ariane { $version } è aggiornata.
update-now-failed = GitHub non ha risposto: riprova più tardi.
...
stats-quotas = Limiti di utilizzo
stats-quota-window = { $minutes ->
        [300] Finestra di 5 ore
        [10080] Settimana
       *[other] Finestra di { $hours } ore
    }
stats-quota-refused = Raggiunto
stats-quota-resets = si azzera il { $date }
stats-quota-reset-since = azzerato dal { $date }
stats-quota-read = Letto il { $date }
stats-quota-plan = piano «{ $plan }»
stats-quota-limit = limite «{ $limit }»
stats-quota-credits = { $balance } crediti rimasti
stats-quota-no-credits = nessun credito
stats-quota-unlimited = crediti illimitati
stats-quota-reached = { $minutes ->
        [300] Limite di 5 ore raggiunto
        [10080] Limite settimanale raggiunto
       *[other] Limite di { $hours } ore raggiunto
    } { $count ->
        [one] una volta
       *[other] { $count } volte
    }, l’ultima il { $date }.
stats-quota-history = { $minutes ->
        [300] Le finestre di 5 ore lette, ognuna al suo valore più alto
        [10080] Le settimane lette, ognuna al suo valore più alto
       *[other] Le finestre di { $hours } ore lette, ognuna al suo valore più alto
    }
stats-quota-ending = Finestra che termina il { $date }
stats-quota-col-end = Fine della finestra
stats-quota-col-used = Lettura più alta
stats-quota-note = Una percentuale è una lettura, non un conteggio: ogni finestra mostra la più alta, con la data in cui è stata letta. Ariane non interroga alcun server: legge ciò che gli assistenti hanno scritto — Codex a ogni risposta; Claude nell’ultima lettura che ha conservato, nella cronologia di Claude Desktop se è installato e in ogni richiesta che i suoi limiti hanno rifiutato.
