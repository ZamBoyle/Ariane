...
convo-span =
    .title =
        ...
        Laatste bericht: { $end }
convo-duration =
    .title = Tijd van het eerste tot het laatste bericht
...
settings-text-size = Tekstgrootte
settings-text-size-select =
    .aria-label = Grootte van de tekst in de hele app
settings-update-now = Nu controleren
update-now-checking = Bezig met controleren…
update-now-current = Ariane { $version } is up-to-date.
update-now-failed = GitHub gaf geen antwoord: probeer het later opnieuw.
...
stats-records = De zijbalk telt { $records } records: { $empty } daarvan bevatten niets om te tonen.
