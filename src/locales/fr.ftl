...
convo-span =
    .title =
        Premier message : { $start }
        Dernier message : { $end }
convo-duration =
    .title = Temps écoulé du premier message au dernier
convo-message-count = { $n ->
        [one] { $n } message
       *[other] { $n } messages
    }
convo-part = Partie { $n } sur { $total }
    .title = Cette conversation se poursuit dans une autre transcription : compactée, reprise ou dupliquée
convo-part-previous =
...
convo-copied = { $count ->
    [one] Cette conversation a commencé par recopier un message de « { $title } » : il se lit là-bas, il n'est pas répété ici.
...
settings-text-size-select =
    .aria-label = Taille du texte dans toute l'application
settings-update-now = Vérifier maintenant
update-now-checking = Vérification…
update-now-current = Ariane { $version } est à jour.
update-now-failed = GitHub n'a pas répondu : réessayez plus tard.
...
stats-records = La barre latérale compte { $records } enregistrements : { $empty } ne contiennent rien à afficher.
