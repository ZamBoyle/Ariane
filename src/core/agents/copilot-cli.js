  ...
  async *read(descriptor, { cursor = null } = {}) {
    ...
    for await (const record of readRecords(descriptor.logFile, { start })) {
      ...
      if (item.kind === 'ignored' && item.part) {
        yield { item: toolMessage(item.part, str(record.value && record.value.timestamp)), cursor: next };
        continue;
      }
...
async function readWorkspace(file) {
  ...
  for (const line of text.split('\n')) {
    ...
    out[field] = unquote(match[2].trim());
  }
  return out;
}

function unquote(value) {
  if (value.length >= 2 && /^(".*"|'.*')$/s.test(value)) return value.slice(1, -1);
  return value;
}
